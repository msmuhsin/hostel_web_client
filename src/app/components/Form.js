...
const validationSchema = Yup.object({
  ...
  Quota: Yup.string().oneOf(["BPL", "SC", "ST", "OEC", "OBC", "PH"]).required("Quota is required"),
  Income: Yup.number().when('Quota', {
    is: 'BPL',
    then: Yup.number().max(100000, 'Income cannot exceed 100000 for BPL quota'),
    otherwise: Yup.number().required('Income is required')
  }),
          ...
          <Form className="bg-white w-full md:w-1/2 rounded-md px-8 py-10 shadow-md flex flex-col space-y-4">
            ...
            <div className="flex flex-col space-y-1">
              ...
              <Input
                ...
                onBlur={handleBlur}
              />
              {touched.DOB && errors.DOB && (https://prod.liveshare.vsengsaas.visualstudio.com/join?8181D87931512C7A7A371FE6DF1F6F8958C4
                <ErrorMessage
                  name="DOB"
                  component="div"
                  className="text-red-500 text-xs"
